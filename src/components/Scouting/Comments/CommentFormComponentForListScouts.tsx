...
const CommentFormComponentForListScouts = ({
  afterCommentAdd,
  replyThreadEvent,
  attachmentDetails,
}: any) => {
  ...
  const addComment = async () => {
    ...
    try {
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/scouts/${attachmentDetails?.scout_id}/attachments/${attachmentDetails?._id}/comment`,
        options
      );
  ...
  const replyThreads = async (comment_id: any) => {
    ...
    try {
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/scouts/${router.query.scout_id}/comments/${comment_id}/reply`,
        options
      );
      let responseData = await response.json();
      if (responseData.success == true) {
  ...
  const fileUploadEvent = async (
    ...
    let obj = {
      attachment: {
        ...
        farm_id: attachmentDetails.farm_id?._id,
      },
    };
...
export default CommentFormComponentForListScouts;
