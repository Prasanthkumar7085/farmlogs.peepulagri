...
import ThreadsForProcurement from "./threadsforProcurement";
const DrawerCommentsForProcurment = ({ drawerClose, rowDetails, drawerOpen }: any) => {
  ...
  const getAllScoutComments = async () => {
    ...
    try {
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/procurement-requests/comment/${rowDetails?._id}/list`,
        options
      );
  ...
  const deleteComment = async (commnet_id: any) => {
    ...
    try {
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/procurement-requests/${rowDetails?._id}/comment/${commnet_id}`,
        options
      );
      let responseData = await response.json();
      if (responseData.success == true) {
...
export default DrawerCommentsForProcurment;
